...
- `DecimalApproximation` / `ComplexDecimalApproximation` can now participate directly in ordinary `+ - * /` and unary `-`, with exact `Number` operands treated as point intervals. If propagated enclosures cannot sustain the input digit request, output digits are conservatively reduced and reflected by `accuracy` / `precision`. `N[N[Pi,20],100]` preserves the original 20-digit guarantee rather than inventing missing digits
