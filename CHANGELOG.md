...
- Added exact real algebraic `root[{a0,...,an},k]` and a `RealAlgebraicNumber` specialized IR. Coefficients are ascending-power exact Rationals and `k` is the 1-based index among distinct increasing real roots. Defining polynomials normalize to monic square-free form; exact Rational Sturm sequences count and isolate roots in Rational intervals, and `N[root[...,k],p]` refines them certifiedly. Real-domain high-degree Rational-polynomial Solve falls back to Root only when existing exact solvers remain unresolved; general Complex root isolation, algebraic-field arithmetic, and minimal-polynomial reduction remain explicit future work.
