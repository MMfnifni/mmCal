...
`N`で得た近似値は，単なる表示文字列だけでなく，真値を必ず含む保証区間(certified enclosure)などの精度情報を保持する。そのため近似値同士，または近似値とexact Numberの`+ - * /`も保証区間を保ったまま計算でき，誤差伝播で保証可能な桁数が減る場合は結果のaccuracyも自動的に低下する。`N[N[Pi,20],100]`のように外側の`N`を増桁しても，元の近似値に存在しない情報やbackend内部のguard桁を新しいAccuracyとして復元しない。
