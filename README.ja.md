...
| `lgamma(x)` | logガンマ関数 | `lgamma(5)=3.178053830348...` |
| `erf(x)`    | 誤差関数      | `erf(1)=0.842700792949...`    |
| `erfc(x)`   | 相補誤差関数  | `erfc(1)=0.157299207050...`   |
...
- こちらは`x`をラジアンとして解釈します
- 信号処理系の「sinc」が欲しいときは基本こちらです
...
| `kurt(...)`            | 超過尖度（正規分布 -> 0）               | `kurt(-1,-0.5,0,0.5,1) < 0`（裾が軽い）<br>`kurt(0,0,0,0,0,10) >> 0`（裾が重い）<br>`kurt(-2,-1,0,1,2) < 0`（一様分布っぽい）<br>`kurt(0,0,0,0,0,-2,2) > 0`（外れ値の影響が強い） |
| `isprime(x)`           | 素数判定<br>（0: 合成数 / 1: 素数）     | `isprime(67280421310721)=1`                                                                                                                                                       |
| `cv(...)`              | 変動係数 = stddev/mean                  | `cv(10,10,10)=0`                                                                                                                                                                  |
| `stderr(...)`          | 標準誤差 = stddev/sqrt(n)               | `stderr(1,2,3)=0.471...`                                                                                                                                                          |
...
| `re(z)`          | 実部                      | `re(3+4I)=3`           |
| `im(z)`          | 虚部                      | `im(3+4I)=4`           |
| `arg(z)`         | 偏角（degree）            | `arg(1+I)=45`          |
| `conj(z)`        | 共役                      | `conj(3+4I)=3-4I`      |
| `polar(r,theta)` | r∠θ（degree）             | `polar(2,60)=1+1.732I` |
...
| 関数名             | 説明                                                      | 入力と出力例                             |
| ------------------ | --------------------------------------------------------- | ---------------------------------------- |
| `rand()`           | 乱数 \lbrack 0,1)                                         | `rand()=0.37...`                         |
| `randn()`          | 正規乱数 N(0,1)                                           | `randn()=-0.23...`                       |
| `randn(mu)`        | 正規乱数 N(mu,1)                                          | `randn(10)=9.61...`                      |
| `randn(mu,sigma)`  | 正規乱数 N(mu,sigma)                                      | `randn(0,2)=1.74...`                     |
| `rand(hi)`         | 乱数 \lbrack 0,5)                                         | `rand(5)=3.37...` ただし0のとき0         |
| `rand(lo,hi)`      | 乱数 \lbrack lo,hi)                                       | `rand(-2,2)=-0.71...`                    |
| `randint()`        | 乱数 0 ro 1                                               | `randint()=0`                            |
| `randint(a)`       | 乱数 \lbrack 0,a\rbrack `a>0`<br> lbrack a,0\rbrack `a<0` | `randint(5)=2`                           |
| `randint(a,b)`     | 乱数 \lbrack a,b\rbrack                                   | `randint(1,6)=3` サイコロ                |
| `choice(a, b,...)` | 1つ抽出                                                   | `choice(2,3,5,7,9,11,13,17,19,23,29)=17` |
...
- 空白は原則無視されますが，数値間では暗黙乗算として処理されます。`3 2` -> `3*2`
...
- AngleModeの対応(sin(30deg), setAngle=RADIAN)
