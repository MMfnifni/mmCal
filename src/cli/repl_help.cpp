...
constexpr FunctionHelpEntry functionHelpEntries[] = {
    ...
    HELP(VectorRejection,
        "Returns the component of a vector orthogonal to a direction.",
        "rejection[vector, onto]",
        "vector, onto: Equal-length rank-1 real or complex arrays; onto must be nonzero.",
        "rejection[{1, 2}, {1, 0}]  ->  {0, 2}"),
    HELP_NOTE(OrthogonalQ,
        "Tests whether the rows of a matrix are pairwise Hermitian-orthogonal.",
        "orthogonalQ[vectors]",
        "vectors: A rank-2 array whose rows are vectors of a common dimension.",
        "Zero rows are allowed; this predicate tests pairwise orthogonality, not linear independence.",
        "orthogonalQ[{{1, 0}, {0, I}}]  ->  True"),
    HELP_NOTE(OrthonormalQ,
        "Tests whether the rows of a matrix are Hermitian-orthonormal.",
        "orthonormalQ[vectors]",
        "vectors: A rank-2 array whose rows are vectors of a common dimension.",
        "Each row must have Hermitian norm 1 and distinct rows must have zero Hermitian inner product.",
        "orthonormalQ[{{1, 0}, {0, I}}]  ->  True"),
    HELP_NOTE(LinearIndependentQ,
        "Tests whether the rows of a matrix are linearly independent.",
        "linearIndependentQ[vectors]",
        "vectors: A rank-2 array whose rows are vectors of a common dimension.",
        "Exact zero/nonzero decisions are required; unresolved symbolic dependence remains unevaluated.",
        "linearIndependentQ[{{1, 2}, {2, 4}}]  ->  False"),
    HELP_NOTE(GramSchmidt,
        "Builds an orthonormal basis for the row span using the Hermitian inner product.",
        "gramSchmidt[vectors]",
        "vectors: A rank-2 array whose rows are real or complex vectors of a common dimension.",
        "Provably dependent rows are dropped. If a required symbolic zero/nonzero decision is unknown, the expression remains unevaluated.",
        "gramSchmidt[{{1, 1}, {1, -1}}]  ->  {{sqrt[2]/2, sqrt[2]/2}, {sqrt[2]/2, -sqrt[2]/2}}"),
    HELP(Gradient,
    ...
    HELP_NOTE(Curl,
        "Computes the Cartesian curl of a two- or three-dimensional vector field.",
        "curl[field, variables]",
        "field: A rank-1 array with 2 or 3 components.\n"
        "variables: The matching 2 or 3 distinct coordinate symbols.",
        "For 2D fields curl[{P,Q},{x,y}] returns the scalar dQ/dx-dP/dy. For 3D fields it returns the usual vector curl.",
        "curl[{-y, x}, {x, y}]  ->  2\n"
        "curl[{0, 0, x*y}, {x, y, z}]  ->  {x, -y, 0}"),
    HELP_NOTE(Laplacian,
        "Computes the Cartesian Laplacian of a scalar or vector field.",
        "laplacian[field, {x1, x2, ...}]",
        "field: A scalar expression or rank-1 vector field.\n"
        "variables: A nonempty rank-1 array of distinct coordinate symbols.",
        "For a vector field the scalar Laplacian is applied componentwise.",
    ...
    HELP_NOTE(DirectionalDerivative,
        "Computes a directional derivative in Cartesian coordinates.",
        "directionalDerivative[field, direction, {x1, x2, ...}]",
        "field: A scalar symbolic expression.\n"
        "direction: A rank-1 array with one component per coordinate.\n"
        "variables: A nonempty rank-1 array of distinct coordinate symbols.",
        "The direction is not normalized automatically: this is the bilinear contraction direction dot grad[field]. Pass normalize[direction] when a unit direction is required.",
